...
class Pact extends React.Component {
    ...
    getMsg(){
        var _that=this;
        var accountId =this.props._params;
        var nationalId=this.props._nationalId;
    ...
    get_creditModel(even){
        ...
        let _that=this;
        if(!this.props._params && !this.state.loanMap.loanNumber){
            return;
        }
        if(_next.hasClass("hidden")){ //显示 creditModel结果
            $.ajax({
                ...
                data:{
                    ...
                    loannumber:this.state.loanMap.loanNumber
                },
                success:function(res) {
                    if(!commonJs.ajaxGetCode(res)){
                        return;
    ...
    componentDidMount (){
        ...
        $(".loansBox li").click(function () {
            ...
            let _accountId=that.props._params;
    ...
    transferHandle(event){
        ...
        if(processing_type=="transfer"){   //转账设定付款
            ...
            var result = confirm("设定扣款时间为"+_time.format('YYYY-MM-DD HH:mm:ss')+"，金额为"+_set_amount+"，type类型为："+set_type_text+"，请确认！");  
            if(!result){  
                return false;
            }
        ...
        data.accountId=this.props._params;
    ...
    savePactstatus(type,event){
        ...
        if(type=="ProcessingStatus"){   //Processing状态
            ...
            _data.accountId=this.props._params;
    ...
    render() {
        ...
        let accountId = this.props._params;
        let {customerId,loanNumber}=this.state;
        return (
                                ...
                                (loanInfoDTOs&&loanInfoDTOs.length>0)?loanInfoDTOs.map((rpy,index)=>{
                                    return <tr key={index} onClick={this.loanDetail.bind(this,rpy.loanNumber)}>
                                                ...
                                                <td className="yellow-font">
                                                    <a target="_blank" className="mr10" href={"/node/down/loan?accountId="+this.props._params+"&loannumber="+rpy.loanNumber}>下载</a>
                                                    {
                                                        (rpy.loanStatus&&rpy.loanStatus.name=="paid_off")?<a target="_blank" 
                                                            href={"/node/loan/settle/proof?accountId="+this.props._params+"&loanNumber="+rpy.loanNumber+"&productNo="+rpy.productNO}>结清</a>:""
                                                    }
                                                </td>
                                            </tr>
                                }):<tr><td>暂未查到数据...</td></tr>
