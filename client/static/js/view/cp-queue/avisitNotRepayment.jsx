...
class AvisitNotRepayment extends React.Component {
    constructor(props){
        super(props);
        ...
        $.ajax({
             ...
             success:function(res){
                $("#loading").remove();
                 ...
                 let _data=res.data;
                 let _visitDataInfoDtos=_data.shouldPayDataInfoDTOS?_data.shouldPayDataInfoDTOS:[]; //搜索结果list
                 that.setState({
    ...
    distribute(){
        ...
        _obj={
            ...
            code:_code,
            bindBy:_bindBy,
            fromWhere:"shouldpaid"
        };
        $.ajax({
            type:"post", 
            url:"/node/reV/assignShouldPay", 
            async:true,
            dataType: "JSON", 
    ...
    render() {
        ...
        return (
            <div  className="content" id="content">
                {/* 搜索条件 */}
                <div data-isresetdiv="yes" className="bar return-visit-condition clearfix pb10 pr20 pl20">
                    ...
                    <dl className="left mt10">
                        <dt>是否回访</dt>
                        <dd>
                            <select className="select-gray whetherRevisit" id='whetherRevisit' name="" id="">
                                <option value="" hidden>请选择</option>
                                <option value="">全部</option>
                    ...
                    <table className="pt-table layout-fixed workAllot-list">
                        <thead>
                            <tr className="th-bg">
                                ...
                                <td width="10%">任务所有者</td>
                                <th width="10%">
                                    <i className="myCheckbox myCheckbox-normal left mr5 mt5" id='selectAll' onClick={this.selectAll.bind(this)}></i>
                                    <a  data-btn-rule="RULE:SHOULDPAID:ASSIGN:TREE" className="btn-blue left" onClick={this.showDistributePop.bind(this)}>分配</a>
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                            {
                                (visitDataInfoDtos && visitDataInfoDtos.length>0)?visitDataInfoDtos.map((repy,i)=>{
                                    return <tr key={i}>
                                                <td width="3%">{i+1}</td>
                                                <td width="10%" title={commonJs.is_obj_exist(repy.loanNumber)}>{commonJs.is_obj_exist(repy.loanNumber)}</td>
                                                ...
                                                <td width="10%" title={commonJs.is_obj_exist(repy.originalDueDate)}>{commonJs.is_obj_exist(repy.originalDueDate)}</td>
...
export default AvisitNotRepayment;