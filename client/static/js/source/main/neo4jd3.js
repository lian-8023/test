...
function Neo4jD3(_selector, _options) {
    ...
    function appendInfoElement(cls, isNode, property, value) {
        ...
        if(isNode){
            property.forEach(v=>{
                strongHtml += '<strong>' + v + '</strong>';
            })
        }else{
            strongHtml = '<strong>' + property + '</strong>'
        }
    ...
    function showText(node) {
        ...
        var text = node.append("text")
            ...
            text.attr('x',function(d,i,v){
                var name = d.properties.name;
                // console.log(d3.selectAll(v[i]))
                    //如果小于四个字符，不换行
                    if (name.length < 4) {
                        d3.selectAll(text._groups[i]).append('tspan')
                        ...
                        var top = d.properties.name.substring(0, 3);
                        d3.select(text._groups[0][i]).append('tspan')
                            .attr("dx", '-0.2em')
                            .attr("dy", 0)
                            .style("font-size","12px")
                            .text(function () {
    ...
    function appendRelationship() {
        return relationship.enter()
                           ...
                           .attr('x',function(d,i,data){
                                let length = 0;
                                data.forEach((v,j)=>{
                                    if(v.__data__.target == d.target&&v.__data__.source == d.source){
                                        length++
                                        d.size = length;
                                    }
                                })
...
//                           .force('y', d3.force().strength(0.002))
    ...
    function neo4jDataToD3Data(data) {
        ...
        //数据分类
        let dataArr = [];
        graph.relationships.map(mapItem => {
        if (dataArr.length == 0) {
            dataArr.push({type:mapItem.type,startNode: mapItem.startNode,endNode:mapItem.endNode,List: [mapItem] })
        } else {
            let res = dataArr.some(item=> {//判断相同，有就添加到当前项
                if (item.startNode == mapItem.startNode&&item.endNode==mapItem.endNode) {
        ...
        dataArr.forEach((v,i)=>{
            var length = 0;
            v.List.length%2 == 1?length=(v.List.length+1)/2:length=v.List.length/2;
            // console.log(length)
            v.List.forEach((e,j)=>{
                e.IDcard = i;
                e.lengthCode = (j+1)-length;
                relationshipsArr.push(e);
            })
        })
        graph.relationships = relationshipsArr;
        return graph;
    }
...
*/
    ...
    function tickRelationships() {
        ...
        let size = 1;
        if (relationship) {
            relationship.attr('transform', function(d,i) {
        ...
        relationship.each(function(relationship) {
            ...
            outline.attr('d', function(d) {
                if(d.lengthCode !== 0){
                    ...
                    //中间坐标
                    rotatedPointB1 = {x:(d.textx- (textBoundingBox.width + textPadding)/2)/2,y:(d.texty) },
                    //结尾坐标
                    rotatedPointD1 = {x:(d.textx- (textBoundingBox.width + textPadding)/2),y:(d.texty) },
                    //另一边开始
                    rotatedPointA2 = {x:rotatedPointD1.x+(textBoundingBox.width + textPadding*2),y:rotatedPointD1.y},
                    //中间坐标
                    rotatedPointB2 = {x:rotatedPointA2.x+(textBoundingBox.width)/2,y:(d.texty) },
                    //结尾坐标
                    rotatedPointD2 = rotatePoint(center, { x: d.target.x - d.source.x - (options.nodeRadius + 1) * u.x - n.x + (n.x - u.x) * options.arrowSize, y: d.target.y - d.source.y - (options.nodeRadius + 1) * u.y - n.y + (n.y - u.y) * options.arrowSize }, angle);
                    rotatedPointD2.y = rotatedPointD2.y+3
                    // console.log(dr)
                    //return "M" + d.source.x + "," + d.source.y + "A" + dr + "," + dr + " 0 0,1 " + d.target.x + "," + d.target.y;
                    return 'M' + rotatedPointA1.x + ' ' + rotatedPointA1.y +
                            ' S ' + rotatedPointB1.x+ ' ' + rotatedPointB1.y + ','+
                                + rotatedPointD1.x+ ' ' + rotatedPointD1.y +
                            // ' Z '+
                            ' M ' + rotatedPointA2.x + ' ' + rotatedPointA2.y+
                            ' S ' + rotatedPointB2.x+ ' ' + rotatedPointB2.y + ','+
                            + rotatedPointD2.x+ ' ' + rotatedPointD2.y
                }else{
                    var center = { x: 0, y: 0 },
                    angle = rotation(d.source, d.target),
                    textBoundingBox = text.node().getBBox(),
    ...
    function tickRelationshipsOverlays() {
        relationshipOverlay.attr('d', function(d) {
            ...
            return 'M ' + rotatedPointA.x+100 + ' ' + rotatedPointA.y +
    ...
    function tickRelationshipsTexts() {
        ...
        relationshipText.attr('transform', function(d,i) {
            // console.log(d)
            var angle = (rotation(d.source, d.target) + 360) % 360,
                ...
                var addY = d.lengthCode*20;
                rotatedPoint.y += addY;
                // console.log(d);
                // console.log(this)
                d.textx = rotatedPoint.x;
                d.texty = rotatedPoint.y;
