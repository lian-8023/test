//@ sourceURL=XYH_repaymentList.js
...
function dealDebitting(data,currPage,pageSize){
    $(".xyh_repaymentTab tbody").html("");
    if(!data || data.length<=0){
        $(".xyh_repaymentTab tbody").html('<tr><th colspan="12">暂未查到相关数据...</th></tr>');
        return;
    }
    let dom="";
    for(var i=0;i<data.length;i++){
        if(i>=pageSize*(currPage-1) && i<=(pageSize*currPage-1)){
            ...
            dom+='<tr>'+
                    ...
                    '    <td>'+is_obj_exist(data_i.dueDate)+'</td>'+
                    '    <td>'+is_obj_exist(data_i.installmentNumber)+'</td>'+
                    '    <td>'+is_obj_exist(data_i.installmentinterestStartDate)+'</td>'+
                    '    <td>'+is_obj_exist(data_i.interest)+'</td>'+
                    '    <td>'+is_obj_exist(data_i.principal)+'</td>'+
                    '    <td>'+is_obj_exist(data_i.rawAddTime)+'</td>'+
                    '    <td>'+is_obj_exist(data_i.rawUpdateTime)+'</td>'+
                    '    <td>'+is_obj_exist(data_i.remainingBlance)+'</td>'+
                    '    <td>'+is_obj_exist(data_i.revealAmount)+'</td>'+
                    '    <td>'+is_obj_exist(data_i.revealLnterest)+'</td>'+
                    '    <td>'+is_obj_exist(data_i.servicesCharge)+'</td>'+
                    '</tr>';
            $(".xyh_repaymentTab tbody").html(dom);
        }
    }
}
/**
...
function getRecordList(customUrls){
    ...
    $.ajax({
        ...
        success:function(res) {
            ...
            listData=_getData.paymentPlanDetailDTOList?_getData.paymentPlanDetailDTOList:[];  //返回接口数据
